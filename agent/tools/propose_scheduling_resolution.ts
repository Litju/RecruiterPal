...
export default defineTool({
  description:
    "Propose a calendar booking or reschedule for an interview after checking the interview belongs to the current organization. Human approval is always required.",
  inputSchema,
  ...
  async execute(input, ctx) {
    ...
    const actionType =
      interview.status === "SCHEDULED" ? "reschedule_interview" : "book_calendar_event";
    ...
    return {
      ok: true,
      proposal,
      uiIntent: { type: "OPEN_SCHEDULING_RESOLUTION", targetId: interview.id },
    };
  },
});
