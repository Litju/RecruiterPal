...
export default defineTool({
  description:
    "Read the latest deterministic decision-readiness snapshot for an application; never calculate or invent a fit score.",
  inputSchema: idInput,
  ...
  async execute(input, ctx) {
    ...
    return readWithTenant(access, async (tx) => {
      ...
      const [snapshot] = await tx
        .select()
        .from(s.decisionReadinessSnapshots)
        .where(
          and(
            eq(s.decisionReadinessSnapshots.applicationId, application.id),
            eq(s.decisionReadinessSnapshots.organizationId, access.tenant.organizationId),
          ),
        )
        .orderBy(desc(s.decisionReadinessSnapshots.computedAt))
        .limit(1);
      return {
        ok: true,
        data: { application, snapshot: snapshot ?? null, humanDecisionRequired: true },
      };
    });
  },
});
