...
export default defineTool({
  description:
    "Read a candidate's recruiting profile and applications in the current organization. Protected demographics are never returned.",
  inputSchema: idInput,
  ...
  async execute(input, ctx) {
    ...
    return readWithTenant(access, async (tx) => {
      ...
      const applications = await tx
        ...
        .where(
          and(
            eq(s.applications.candidateId, candidate.id),
