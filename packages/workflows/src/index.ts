...
function planScorecardChase(input: ParsedWorkflowInput, snapshot: WorkflowSnapshot): WorkflowPlan {
  ...
  if (snapshot.interviewStatus !== "COMPLETED" || submitted >= required) {
    return {
      status: "COMPLETE",
      nextWakeAt: null,
      steps: snapshot.activeObligationId
        ? [
            {
              kind: "resolve_obligation",
              key: "resolve-scorecard-obligation",
              obligationId: snapshot.activeObligationId,
            },
          ]
        : [],
    };
  ...
  const steps: WorkflowPlanStep[] = [
    {
      kind: "obligation",
      key: "scorecard-obligation",
      obligationType: "SCORECARD_SUBMISSION",
      dueAt,
      summary: `${required - submitted} scorecard(s) remain after the interview.`,
    },
  ];
  ...
  if ((snapshot.followUpAttempt ?? 0) < 1) {
    steps.push({
      kind: "action",
      key: "scorecard-reminder-1",
      actionType: "send_scorecard_reminder",
      summary: "Send the first scorecard reminder.",
    });
    return { status: "WAITING", nextWakeAt: addMs(snapshot.now, input.escalationPeriodMs), steps };
  }
  steps.push({
    kind: "exception",
    key: "scorecard-escalation",
    type: "MISSING_SCORECARD",
    severity: "HIGH",
    title: "Required scorecard is overdue",
    detail: `${required - submitted} required scorecard(s) remain missing after the reminder.`,
  });
  return { status: "BLOCKED", nextWakeAt: null, steps };
}

function planCandidateFollowUp(
  input: ParsedWorkflowInput,
    ...
    return {
      ...
      steps: snapshot.activeObligationId
        ? [
            {
              kind: "resolve_obligation",
              key: "resolve-candidate-obligation",
              obligationId: snapshot.activeObligationId,
            },
          ]
        : [],
  ...
  const steps: WorkflowPlanStep[] = [
    {
      kind: "obligation",
      key: "candidate-response-obligation",
      obligationType: "CANDIDATE_RESPONSE",
      dueAt,
      summary: "Candidate response is pending.",
    },
  ];
  if (snapshot.now < dueAt) return { status: "WAITING", nextWakeAt: dueAt, steps };
  ...
  if (attempt < input.maxAttempts) {
    steps.push({
      kind: "action",
      key: `candidate-follow-up-${attempt + 1}`,
      actionType: "send_candidate_follow_up",
      summary: `Send approved follow-up attempt ${attempt + 1}.`,
    });
    return { status: "WAITING", nextWakeAt: addMs(snapshot.now, input.escalationPeriodMs), steps };
  }
  steps.push({
    kind: "exception",
    key: "candidate-follow-up-escalation",
    type: "CANDIDATE_RESPONSE_OVERDUE",
    severity: "HIGH",
    title: "Candidate response is overdue",
    detail: "The approved follow-up limit was reached without a candidate response.",
  });
  return { status: "BLOCKED", nextWakeAt: null, steps };
}

function planScheduling(snapshot: WorkflowSnapshot): WorkflowPlan {
  if ((snapshot.declinedParticipantCount ?? 0) === 0 && !snapshot.noFeasibleSlot)
    return { status: "COMPLETE", nextWakeAt: null, steps: [] };
  if (snapshot.noFeasibleSlot) {
    return {
      ...
      steps: [
        {
          kind: "exception",
          key: "scheduling-no-feasible-slot",
          type: "SCHEDULING_CONFLICT",
          severity: "HIGH",
          title: "No feasible interview slot",
          detail:
            "Deterministic availability reconciliation found no valid slot within the current constraints.",
        },
      ],
    };
  }
  return {
    ...
    steps: [
      {
        kind: "approval",
        key: "scheduling-booking-approval",
        actionType: "book_calendar_event",
        requiredPermission: "interview:write",
        rationale: "Approve the deterministic slot and notify the interview panel.",
      },
    ],
  };
}
...
function planReplacement(snapshot: WorkflowSnapshot): WorkflowPlan {
  if ((snapshot.declinedParticipantCount ?? 0) === 0)
    return { status: "COMPLETE", nextWakeAt: null, steps: [] };
  return {
    status: "WAITING",
    nextWakeAt: null,
    steps: [
      {
        kind: "approval",
        key: "interviewer-replacement-approval",
        actionType: "request_interviewer_substitution",
        requiredPermission: "interview:write",
        rationale: "A required interviewer declined; approve a replacement from the approved pool.",
      },
    ],
  };
}
    ...
    steps: [
      {
        ...
        type: "INTEGRATION_DRIFT",
        severity: "MEDIUM",
        title: "Integration reconciliation required",
        detail:
          "External and canonical state are stale or inconsistent; no external write was attempted.",
      },
    ],
  };
}
...
function planStaleExceptions(snapshot: WorkflowSnapshot): WorkflowPlan {
  if ((snapshot.staleExceptionCount ?? 0) === 0)
    return { status: "COMPLETE", nextWakeAt: null, steps: [] };
  return {
    status: "BLOCKED",
    nextWakeAt: null,
    steps: [
      {
        kind: "exception",
        key: "stale-exception-reconciliation",
        type: "STALE_EXCEPTION_STATE",
        severity: "MEDIUM",
        title: "Exception state needs reconciliation",
        detail: `${snapshot.staleExceptionCount} open exception(s) have not been recomputed within the policy window.`,
      },
    ],
  };
}
...
function planDeadline(input: ParsedWorkflowInput, snapshot: WorkflowSnapshot): WorkflowPlan {
  ...
  if (!deadline || deadline.getTime() > snapshot.now.getTime() + input.escalationPeriodMs)
    return { status: "COMPLETE", nextWakeAt: deadline ?? null, steps: [] };
  return {
    status: "BLOCKED",
    nextWakeAt: null,
    steps: [
      {
        kind: "exception",
        key: "competing-offer-deadline",
        type: "COMPETING_OFFER_DEADLINE",
        severity: "CRITICAL",
        title: "Competing offer deadline is near",
        detail:
          "A confirmed candidate-stated deadline is within the escalation window. Human review remains required.",
        deadlineAt: deadline,
      },
    ],
  };
...
export function buildWorkflowPlan(
  ...
  switch (input.workflowType) {
    case "scorecard_chase":
      return planScorecardChase(input, snapshot);
    case "candidate_follow_up":
      return planCandidateFollowUp(input, snapshot);
    case "scheduling_resolution":
      return planScheduling(snapshot);
    case "interviewer_decline_replacement":
      return planReplacement(snapshot);
    case "integration_reconciliation":
      return planIntegration(snapshot);
    case "stale_exception_reconciliation":
      return planStaleExceptions(snapshot);
    case "deadline_candidate_escalation":
      return planDeadline(input, snapshot);
  }
...
function applicationContext(input: ParsedWorkflowInput): ApplicationContext {
  ...
  return {
    ...
    actor,
    correlationId: workflowKey(input),
  };
}
...
async function ensureWorkflowInstance(
  ...
  return withTenant(db, ctx, async (tx) => {
    const [inserted] = await tx
      ...
      .onConflictDoNothing({
        target: [
          schema.workflowInstances.organizationId,
          schema.workflowInstances.workflowType,
          schema.workflowInstances.businessObjectId,
        ],
      })
    ...
    const [existing] = await tx
      ...
      .where(
        and(
          eq(schema.workflowInstances.organizationId, input.organizationId),
          eq(schema.workflowInstances.workflowType, input.workflowType),
          eq(schema.workflowInstances.businessObjectId, input.businessObjectId),
        ),
      )
      .limit(1);
...
async function readSnapshot(
  ...
  return withTenant(
    ...
    async (tx) => {
      ...
      const interviewId =
        input.interviewId ??
        (input.workflowType === "scorecard_chase" ? input.businessObjectId : undefined);
      if (interviewId) {
        ...
        if (interview) {
          ...
          snapshot.noFeasibleSlot = ["AWAITING_AVAILABILITY", "RESCHEDULE_REQUIRED"].includes(
            interview.status,
          );
          const scorecards = await tx
            .select({ status: schema.scorecards.status })
            .from(schema.scorecards)
            .where(
              and(
                eq(schema.scorecards.interviewId, interview.id),
                eq(schema.scorecards.organizationId, input.organizationId),
                inArray(schema.scorecards.status, ["SUBMITTED", "AMENDED"]),
              ),
            );
          snapshot.submittedScorecards = scorecards.length;
        }
        const declined = await tx
          .select({ userId: schema.interviewParticipants.userId })
          .from(schema.interviewParticipants)
          .where(
            and(
              eq(schema.interviewParticipants.interviewId, interviewId),
              eq(schema.interviewParticipants.organizationId, input.organizationId),
              eq(schema.interviewParticipants.declined, true),
            ),
          )
          .limit(20);
      ...
      if (snapshot.applicationId) {
        const [obligation] = await tx
          .select({
            id: schema.applicationObligations.id,
            dueAt: schema.applicationObligations.dueAt,
          })
          .from(schema.applicationObligations)
          .where(
            and(
              eq(schema.applicationObligations.organizationId, input.organizationId),
              eq(schema.applicationObligations.applicationId, snapshot.applicationId),
              inArray(schema.applicationObligations.state, ["PENDING", "ESCALATED"]),
              inArray(schema.applicationObligations.obligationType, [
                "SCORECARD_SUBMISSION",
                "CANDIDATE_RESPONSE",
              ]),
            ),
          )
          .orderBy(schema.applicationObligations.dueAt)
          .limit(1);
        ...
        const followUpActionType =
          input.workflowType === "scorecard_chase"
        ...
        const followUps = await tx
          ...
          .where(
            and(
              eq(schema.actions.organizationId, input.organizationId),
              eq(schema.actions.actionType, followUpActionType),
              eq(schema.actions.status, "SUCCEEDED"),
              eq(schema.actions.targetRefs, [`application:${snapshot.applicationId}`]),
            ),
          );
        ...
        const inbound = await tx
          ...
          .innerJoin(
            schema.communicationThreads,
            eq(schema.communicationThreads.id, schema.messages.threadId),
          )
          .where(
            and(
              eq(schema.communicationThreads.organizationId, input.organizationId),
              eq(schema.communicationThreads.applicationId, snapshot.applicationId),
      ...
      if (input.workflowType === "integration_reconciliation") {
        ...
        const stale = await tx
          ...
          .where(
            and(
              eq(schema.integrationConnections.organizationId, input.organizationId),
              or(
                isNull(schema.integrationConnections.lastSyncedAt),
                lt(schema.integrationConnections.lastSyncedAt, cutoff),
              ),
            ),
          )
          .limit(1);
      ...
      if (input.workflowType === "stale_exception_reconciliation") {
        ...
        const stale = await tx
          ...
          .where(
            and(
              eq(schema.exceptions.organizationId, input.organizationId),
              inArray(schema.exceptions.status, ["OPEN", "ACKNOWLEDGED"]),
              or(
                isNull(schema.exceptions.lastRecomputedAt),
                lt(schema.exceptions.lastRecomputedAt, cutoff),
              ),
            ),
          )
          .limit(20);
      ...
      if (input.workflowType === "deadline_candidate_escalation" && snapshot.applicationId) {
        const [fact] = await tx
          .select({ normalizedValue: schema.extractedFacts.normalizedValue })
          .from(schema.extractedFacts)
          .where(
            and(
              eq(schema.extractedFacts.organizationId, input.organizationId),
              eq(schema.extractedFacts.applicationId, snapshot.applicationId),
              eq(schema.extractedFacts.factType, "COMPETING_OFFER_DEADLINE"),
              eq(schema.extractedFacts.reviewState, "CONFIRMED"),
            ),
          )
          .limit(1);
...
async function syncWorkflowObligation(
  db: RecruiterPalDb,
  input: ParsedWorkflowInput,
  instanceId: string,
  step: Extract<WorkflowPlanStep, { kind: "obligation" }>,
): Promise<void> {
  await withTenant(
    db,
    { organizationId: input.organizationId, userId: input.userId },
    async (tx) => {
      ...
      await tx
        .insert(schema.workflowObligations)
        .values({
          organizationId: input.organizationId,
          instanceId,
          obligationKey,
        ...
        .onConflictDoUpdate({
          target: schema.workflowObligations.obligationKey,
          set: {
            state: "ACTIVE",
            summary: step.summary,
            visibleOnToday: true,
...
async function finishWorkflow(
  ...
  await withTenant(
    ...
    async (tx) => {
      ...
      if (status === "COMPLETE")
        await tx
          .update(schema.workflowObligations)
          .set({ state: "SATISFIED", visibleOnToday: false, updatedAt: new Date() })
          .where(
            and(
              eq(schema.workflowObligations.instanceId, instanceId),
              eq(schema.workflowObligations.organizationId, input.organizationId),
              eq(schema.workflowObligations.state, "ACTIVE"),
            ),
          );
    },
  );
    ...
    if (step.kind === "obligation") {
      ...
      await recordWorkflowAction(db, ctx, {
        ...
        outcome: {
          delivered: false,
          synthetic: true,
          reason: "Adapter boundary records intent; live provider is qualified separately.",
        },
      });
    } else if (step.kind === "exception") {
      await createException(db, ctx, {
        type: step.type,
      ...
      await requestApproval(db, ctx, {
        actionType: step.actionType,
        targetRefs: [
          input.interviewId ? `interview:${input.interviewId}` : `workflow:${workflowKey(input)}`,
        ],
        parameters: { workflowType: input.workflowType, businessObjectId: input.businessObjectId },
...
export async function executeWorkflow(
  ...
  return {
    ...
    status: plan.status,
    replayed: instance.replayed || plan.steps.length === 0,
