...
import { getDb, schema, withTenant, type RecruiterPalDb } from "@recruiterpal/db";
...
type ParsedWorkflowInput = z.output<typeof workflowInputSchema>;
...
export type WorkflowPlanStep =
  ...
  | {
      kind: "approval";
      key: string;
      actionType: "request_interviewer_substitution" | "book_calendar_event" | "reschedule_interview";
...
async function executeWorkflowStep(input: WorkflowInput): Promise<WorkflowResult> {
  "use step";
