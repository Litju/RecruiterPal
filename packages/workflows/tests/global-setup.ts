...
function assertDisposableDatabase(url: string): void {
  ...
  if (!(parsed.hostname === "localhost" || parsed.hostname === "127.0.0.1") || (parsed.port && parsed.port !== "5499")) throw new Error("Refusing destructive DB qualification without RP_TEST_DATABASE_URL pointing at a disposable local database.");
}

export async function setup() {
  assertDisposableDatabase(adminUrl);
  const pool = new Pool({ connectionString: adminUrl, max: 4 });
  const db = drizzle(pool);
  await db.execute("DROP SCHEMA IF EXISTS public CASCADE; DROP SCHEMA IF EXISTS drizzle CASCADE; CREATE SCHEMA public;");
