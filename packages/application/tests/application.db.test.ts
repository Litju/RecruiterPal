...
function actor(
  userId: string,
  role: string,
  origin: ActorContext["origin"] = "human",
): ActorContext {
  return {
    userId,
...
beforeAll(async () => {
  ...
  const [appRow] = await admin.db
    .select({
      id: schema.applications.id,
      protocolVersionId: schema.applications.protocolVersionId,
    })
    .from(schema.applications)
    .where(eq(schema.applications.id, world.sofiaApplicationId!));
...
describe("deterministic application boundary", () => {
  it("rejects stale protocol and invalid stage transitions", async () => {
    await expect(
      transitionApplication(app.db, context(world.recruiters[0]!.id, "recruiter"), {
        applicationId,
  ...
  it("cannot execute a consequential decision from an agent origin", async () => {
    await expect(
      transitionApplication(app.db, context(world.recruiters[0]!.id, "recruiter", "agent"), {
        applicationId,
        protocolVersionId,
        toStage: "REJECTED",
  ...
  it("requires and honors a human approval for an operational transition", async () => {
    const target = await admin.db
      .select({ currentStage: schema.applications.currentStage })
      .from(schema.applications)
    ...
    const applied = await transitionApplication(
      app.db,
      context(world.recruiters[0]!.id, "recruiter"),
      {
        applicationId,
        protocolVersionId,
        toStage: "OFFER",
    ...
    const replay = await transitionApplication(
      app.db,
      context(world.recruiters[0]!.id, "recruiter"),
      {
        applicationId,
  ...
  it("isolates assigned scorecard submission and persists evidence", async () => {
    const [scorecard] = await admin.db
      .select()
      .from(schema.scorecards)
      .where(eq(schema.scorecards.status, "OPEN"))
      .limit(1);
    expect(scorecard).toBeDefined();
    const [protocolComp] = await admin.db
      .select({ protocolVersionId: schema.protocolCompetencies.protocolVersionId })
      .from(schema.protocolCompetencies)
      .where(eq(schema.protocolCompetencies.protocolVersionId, scorecard!.protocolVersionId))
      .limit(1);
    const [candidate] = await admin.db
      ...
      .from(schema.applications)
      .where(eq(schema.applications.id, scorecard!.applicationId));
    const comps = await admin.db
      .select({ competencyId: schema.protocolCompetencies.competencyId })
      .from(schema.protocolCompetencies)
      .where(
        and(
          eq(schema.protocolCompetencies.protocolVersionId, protocolComp!.protocolVersionId),
          eq(schema.protocolCompetencies.isRequired, true),
        ),
      );
    const ratings = comps.map((item) => ({
      competencyId: item.competencyId,
      rating: 4,
      evidenceNote: "Observed a concrete, rubric-anchored example.",
    }));
    const submitted = await submitScorecard(
      app.db,
      context(scorecard!.raterUserId, "interviewer"),
      {
        scorecardId: scorecard!.id,
        protocolVersionId: candidate!.protocolVersionId,
        ratings,
        amend: false,
        idempotencyKey: "submit-open-card",
      },
    );
    expect(submitted.value.status).toBe("SUBMITTED");
    const evidence = await withTenant(
      app.db,
      { organizationId: world.organizationId, userId: scorecard!.raterUserId },
      (tx) =>
        tx
          .select()
          .from(schema.evidenceObservations)
          .where(eq(schema.evidenceObservations.sourceObjectId, scorecard!.id)),
    );
    expect(evidence.length).toBe(comps.length);
    await expect(
      submitScorecard(app.db, context(world.recruiters[0]!.id, "recruiter"), {
        scorecardId: scorecard!.id,
        protocolVersionId: candidate!.protocolVersionId,
        ratings,
        amend: false,
        idempotencyKey: "wrong-rater",
      }),
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
  });

  it("persists readiness, action proposals, and idempotent action records", async () => {
    const readiness = await recomputeDecisionReadiness(
      app.db,
      context(world.recruiters[0]!.id, "recruiter"),
      { applicationId, protocolVersionId },
    );
    expect(["INCOMPLETE", "CONFLICT_REVIEW_REQUIRED", "READY"]).toContain(readiness.result.status);
    const proposed = await proposeAction(
      app.db,
      {
        ...context(world.recruiters[0]!.id, "recruiter", "agent"),
        actor: actor(world.recruiters[0]!.id, "recruiter", "agent"),
      },
      {
    ...
    const replay = await proposeAction(
      app.db,
      {
        ...context(world.recruiters[0]!.id, "recruiter", "agent"),
        actor: actor(world.recruiters[0]!.id, "recruiter", "agent"),
    ...
    const appRows = await withTenant(
      app.db,
      { organizationId: world.organizationId, userId: world.recruiters[0]!.id },
      (tx) =>
        tx.select().from(schema.applications).where(eq(schema.applications.id, applicationId)),
    );
