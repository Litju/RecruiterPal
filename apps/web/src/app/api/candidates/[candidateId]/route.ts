...
export async function GET(
  ...
  const workspace = await withSessionTenant(session, async (tx) => {
    const [candidate] = await tx
      ...
      .from(candidates)
      .where(
        and(eq(candidates.organizationId, session.organizationId), eq(candidates.id, candidateId)),
      )
      .limit(1);
    ...
    const [evidenceRows, scorecardRows, readinessRows, timelineRows] = await Promise.all([
      tx
        .select({
          id: evidenceObservations.id,
          applicationId: evidenceObservations.applicationId,
          competencyId: evidenceObservations.competencyId,
          observation: evidenceObservations.observation,
          rating: evidenceObservations.rating,
          sourceType: evidenceObservations.sourceType,
          provenance: evidenceObservations.provenance,
          observedAt: evidenceObservations.observedAt,
        })
        .from(evidenceObservations)
        .where(
          and(
            eq(evidenceObservations.organizationId, session.organizationId),
            inArray(evidenceObservations.applicationId, applicationIds),
          ),
        )
      ...
      tx
        .select({
          ...
          raterUserId: scorecards.raterUserId,
        ...
        .where(
          and(
            eq(scorecards.organizationId, session.organizationId),
            inArray(scorecards.applicationId, applicationIds),
          ),
        )
        .orderBy(desc(scorecards.submittedAt)),
      tx
        .select({
          id: decisionReadinessSnapshots.id,
          applicationId: decisionReadinessSnapshots.applicationId,
          status: decisionReadinessSnapshots.status,
          reasons: decisionReadinessSnapshots.reasons,
          computedAt: decisionReadinessSnapshots.computedAt,
        })
        .from(decisionReadinessSnapshots)
        .where(
          and(
            eq(decisionReadinessSnapshots.organizationId, session.organizationId),
            inArray(decisionReadinessSnapshots.applicationId, applicationIds),
          ),
        )
      ...
      tx
        .select({
          ...
          fromStage: applicationStageEvents.fromStage,
          toStage: applicationStageEvents.toStage,
          reason: applicationStageEvents.reason,
          actorType: applicationStageEvents.actorType,
          occurredAt: applicationStageEvents.occurredAt,
        })
        .from(applicationStageEvents)
        .where(
          and(
            eq(applicationStageEvents.organizationId, session.organizationId),
            inArray(applicationStageEvents.applicationId, applicationIds),
          ),
        )
      ...
      scorecardIds.length === 0
        ...
        : await tx
            .select({
              ...
              competencyId: scorecardRatings.competencyId,
              rating: scorecardRatings.rating,
              evidenceNote: scorecardRatings.evidenceNote,
              rubricAnchor: scorecardRatings.rubricAnchor,
            })
            .from(scorecardRatings)
            .where(
              and(
                eq(scorecardRatings.organizationId, session.organizationId),
                inArray(scorecardRatings.scorecardId, scorecardIds),
              ),
            );
    const competencyIds = [
      ...new Set([
        ...evidenceRows.map((evidence) => evidence.competencyId),
        ...ratingRows.map((rating) => rating.competencyId),
      ]),
    ];
    const competencyRows = await (competencyIds.length === 0
      ...
      : tx
          ...
          .where(
            and(
              eq(competencies.organizationId, session.organizationId),
              inArray(competencies.id, competencyIds),
            ),
          ));
    const competencyNameById = new Map(
      competencyRows.map((competency) => [competency.id, competency.name]),
    );
    const latestReadiness = new Map<string, (typeof readinessRows)[number]>();
    for (const readiness of readinessRows)
      if (!latestReadiness.has(readiness.applicationId))
        latestReadiness.set(readiness.applicationId, readiness);
    return {
      candidate,
      applications: applicationRows,
      evidence: evidenceRows.map((evidence) => ({
        ...evidence,
        competency: competencyNameById.get(evidence.competencyId) ?? "Unlabeled competency",
      })),
      scorecards: scorecardRows,
      ratings: ratingRows.map((rating) => ({
        ...rating,
        competency: competencyNameById.get(rating.competencyId) ?? "Unlabeled competency",
      })),
