      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
          <main className="min-w-0">
            <section aria-label="Execution summary" className="mb-6 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
              <div className="rounded-card border border-border-subtle bg-surface-1 p-3">
                <p className="text-[11px] uppercase tracking-wide text-text-tertiary">Decision ready</p>
              ...
              <div className="rounded-card border border-border-subtle bg-surface-1 p-3">
                <p className="text-[11px] uppercase tracking-wide text-text-tertiary">Pending obligations</p>
                <p className="mt-1 text-xl font-semibold tabular">{execution.pendingObligations}</p>
                <p className={`text-[12px] ${execution.overdueObligations > 0 ? "text-danger" : "text-text-secondary"}`}>{execution.overdueObligations} overdue</p>
              </div>
              <div className="rounded-card border border-border-subtle bg-surface-1 p-3">
                <p className="text-[11px] uppercase tracking-wide text-text-tertiary">Needs review</p>
              ...
              <div className="rounded-card border border-border-subtle bg-surface-1 p-3">
                <p className="text-[11px] uppercase tracking-wide text-text-tertiary">Durable workflows</p>
            ...
            <ExceptionSection
              ...
              exceptions={[]}
              emptyText={execution.readinessReady > 0 ? `${execution.readinessReady} readiness snapshot(s) are ready for a human decision in Decisions.` : "No decisions are waiting on a human right now. When evidence and approvals are complete, candidates appear here for your call."}
            />
            <ExceptionSection
            ...
            {deadlines.length > 0 ? (
              <section aria-label="Candidate deadlines" className="mb-6">
                <div className="mb-2 flex items-baseline justify-between gap-3">
                  <div>
                    <h2 className="text-[13px] font-semibold uppercase tracking-wide text-text-secondary">Candidate deadlines</h2>
                    <p className="mt-1 text-[12px] text-text-tertiary">Verified deadlines shape urgency; they never decide an outcome.</p>
                  </div>
                  <Link href="/candidates" className="text-[12px] font-medium text-pal-text hover:underline">Open workspaces</Link>
                </div>
                <ul className="divide-y divide-border-subtle rounded-card border border-border-subtle bg-surface-1">
                  {deadlines.map((d) => (
                    <li key={d.id} className="flex items-center justify-between gap-3 px-4 py-3">
                      ...
                      <span className={`shrink-0 text-right text-[12px] tabular ${d.deadlineUrgent ? "font-semibold text-danger" : "text-text-secondary"}`}>
            ...
            <section aria-label="Handled by Pal while you were away" className="mb-6">
              <h2 className="mb-2 text-[13px] font-semibold uppercase tracking-wide text-text-secondary">Handled by Pal</h2>
              {execution.automatedActivity.length === 0 ? (
                <p className="rounded-card border border-dashed border-border-subtle bg-surface-1 px-4 py-5 text-[13px] leading-relaxed text-text-tertiary">Safe administrative actions completed automatically (reminders sent, syncs reconciled) appear here with audit drill-down once durable workflows have run.</p>
              ) : (
                <ol className="space-y-2 rounded-card border border-border-subtle bg-surface-1 p-3" aria-label="Recent Pal and workflow activity">
                  {execution.automatedActivity.map((activity) => (
                    <li key={activity.id} className="flex items-start gap-3 border-l-2 border-pal/40 pl-3 text-[12px]">
                      <div className="min-w-0 flex-1"><p className="font-medium">{activity.actionType.replace(/_/g, " ").toLowerCase()}</p><p className="text-text-tertiary">{activity.actorType.toLowerCase()} · {activity.targetType.toLowerCase()} · {activity.outcome.toLowerCase()}</p></div>
                      <time className="shrink-0 tabular text-text-tertiary">{activity.occurredAt.toLocaleString("en-US", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}</time>
                    </li>
                  ))}
                </ol>
              )}
          ...
          <PalPane
            ...
            automatedActivity={execution.automatedActivity.map((activity) => ({ ...activity, occurredAt: activity.occurredAt.toISOString() }))}
            intent={intent}
          />
        </div>
      </div>
